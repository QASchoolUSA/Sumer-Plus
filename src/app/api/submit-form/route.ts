...
const CORPORATE_SCHEMA: SectionSchema[] = [
    {
...
const PERSONAL_SCHEMA: SectionSchema[] = [
    {
...
function getEmailDisplayValue(value: unknown, field: FieldSchema) {
    ...
    if (field.type === 'select' && field.options) {
        return (typeof value === 'string' && field.options?.[value]) || (typeof value === 'string' ? value : '') || '';
    }
    return value || '';
}

// Helper to get raw value for PDF
function getPdfRawValue(value: unknown, field: FieldSchema) {
    if (field.type === 'checkbox') {
...
function drawCheck(doc: PDFKit.PDFDocument, x: number, y: number, size = 10) {
    doc.save();
...
function drawCross(doc: PDFKit.PDFDocument, x: number, y: number, size = 10) {
    doc.save();
...
function drawCircle(doc: PDFKit.PDFDocument, x: number, y: number, size = 10) {
    doc.save();
...
function createPDFBuffer(type: string, data: Record<string, unknown>, schema: SectionSchema[]) {
    return new Promise((resolve, reject) => {
        ...
        schema.forEach((section: SectionSchema) => {
            ...
            for (let i = 0; i < section.fields.length; i += 2) {
                ...
                if (typeof rawValue1 === 'boolean') {
                    ...
                    const textValue = String(rawValue1 ?? "");
                    const displayValue = textValue.trim() === "" ? "(Empty)" : textValue;
                ...
                if (field2) {
                    ...
                    if (typeof rawValue2 === 'boolean') {
                        ...
                        const textValue = String(rawValue2 ?? "");
                        const displayValue = textValue.trim() === "" ? "(Empty)" : textValue;
