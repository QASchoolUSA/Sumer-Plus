...
import { CONSULTING_QUESTIONS, calculateRisk, type ConsultingAnswers } from './consultingQuestions';
...
export default function PricingCalculator({
    ...
    // Consulting State
    const [consultingAnswers, setConsultingAnswers] = useState<ConsultingAnswers>({});
    const [currentStep, setCurrentStep] = useState(0);
    const [consultingFinished, setConsultingFinished] = useState(false);

    const handleConsultingAnswer = (
        id: string,
    ...
    const handleSituationToggle = (id: string) => {
        setSituations((prev) =>
    ...
    const taxEstimate = useMemo(() => {
        ...
        const statusCost =
            FILING_STATUS_COST[filingStatus as keyof typeof FILING_STATUS_COST] ||
            0;
    ...
    const bookkeepingEstimate = useMemo(() => {
        const base = TX_VOL_COST[txVolume as keyof typeof TX_VOL_COST] || 250;
        const acct =
            ACCT_COUNT_COST[accountCount as keyof typeof ACCT_COUNT_COST] || 0;
        ...
        subtotal += ASSET_COST[assetCount as keyof typeof ASSET_COST] || 0;
                                                        ...
                                                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '15px' }}>
                                                            {field.options?.map(opt => (
                                                                <label key={opt} style={{ display: 'flex', alignItems: 'center', gap: '5px', cursor: 'pointer' }}>
                                                                    <input
                                                                        type="checkbox"
                                                                        checked={Array.isArray(consultingAnswers[field.id]) && (consultingAnswers[field.id] as string[]).includes(opt)}
                                                                        onChange={(e) => {
                                                                            const current = Array.isArray(consultingAnswers[field.id]) ? [...(consultingAnswers[field.id] as string[])] : [];
                                                    ...
                                                    <>
                                                        ...
                                                        {field.type === 'textarea' ? (
                                                            <textarea
                                                                ...
                                                                value={String(consultingAnswers[field.id] ?? '')}
                                                                onChange={(e) => handleConsultingAnswer(field.id, e.target.value)}
                                                            />
                                                        ) : (
                                                            <input
